...
exports.getIp = (position) => {
  ...
  if (position > 0 && position <= ips.address) {
 ...
 * 获取浏览器URL
 */
exports.getBrowerUrl = (port, url) => {
  let browerUrl;
  if (/^(https?:|\/\/)/.test(url)) {
    browerUrl = url;
  } else {
    const host = exports.getHost(port);
    if (url) {
      browerUrl = `${host}/${url}`;
    } else {
      browerUrl = host;
...
exports.getBrowserUrl = (port, url) => {
  let browserUrl;
  if (/^(https?:|\/\/)/.test(url)) {
    browserUrl = url;
  } else {
...
exports.writeFile = (filepath, content) => {
  try {
    ...
    console.error(`writeFile ${filepath} err`, error);
  }
};
...
exports.readFile = (filepath) => {
  try {
    if (fs.existsSync(filepath)) {
      const con = fs.readFileSync(filepath, 'utf8');
      // console.log(con,'读取文件',JSON.parse(content))
...
exports.createHTMLFileTree = (fileList) =>
  fileList.reduce(
    (html, item) =>
      `${html} <a href="${item.pathname}">${item.filename}</a></br>`,
    '',
  );
...
exports.getFileList = (root, pathname) => {
  ...
  if (fs.existsSync(rootFilePath)) {
    if (fs.statSync(rootFilePath).isDirectory()) {
      const fileList = fs.readFileSync(rootFilePath);
      const dirs = [];
      const files = [];
      ...
      return dirs.concat(files).map((file) => {
        const newpathname = file.replace(root, '');
 ...
 * @param {stirng} cmd 命令
 */
exports.exec = (cmd) => shell.exec(cmd);

/**
 * 命令删除
 * @param {string} filepath 删除路径
 */
exports.rm = (filepath) => {
...
exports.httpServer = (cfg, callback) => {
  ...
  httpserver.beforeServe = (request, response, body, encoding) => {
    ...
    textTypes.forEach((ext1) => {
      httpserver.config.contentType[ext1] = 'text/plain';
    });
    ...
    if (response.statusCode === 415) {
      if (fs.existsSync(filepath)) {
        if (fs.statSync(filepath).isDirectory()) {
          response.setHeader(
            'Content-Type',
            httpserver.config.contentType.html,
          );
          response.statusCode = 200;
      ...
      const ext = extKeys.find((ext2) => {
        const file = `${filepath}.${ext2}`;
      ...
      if (ext) {
        ...
        if (ext === 'js') {
          response.setHeader(
            'Content-Type',
            httpserver.config.contentType.json,
          );
          try {
            ...
            console.warn('[utils] http serve error', e);
          }
        }
  ...
  httpserver.deploy(options, (server) => {
    ...
    console.log(
      chalk.green(
        `Http server ${chalk.yellow(url)} or ${chalk.yellow(
          host,
        )} is serve ${chalk.blue(root)}\r\n`,
      ),
    );
